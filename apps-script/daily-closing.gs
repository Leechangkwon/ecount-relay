...
function buildGuideSheet() {
  ...
  add('P~S · U', '일사용량 · 발주 커버일수 · 목표재고 · 발주수량 · 창고 인가량', '일사용량(P)=최근30일 판매÷30(자동, 초기엔 ERP 수불부 대체). 커버일수(Q)=기본 4일(월·수·금 발주, 금→화 입고). 목표재고=P×Q(창고 기준). 창고가용=창고실재고−오늘 중앙보충분(부족수량). 발주=MAX(목표−창고가용, 창고인가량−창고가용)−미입고, 5단위 올림(단, 인가량·목표재고가 모두 10 미만인 저회전 품목은 필요수량 그대로). 연휴 전엔 Q를 늘려서 발주(다음날 자동 원복)', 'line');
  gap();
...
function writeStockFormulas_(main, startRow, n) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    // 발주 단위: 저회전 품목(창고인가량·목표재고 모두 10 미만)은 1단위(정수 올림만), 그 외 5단위 — 소량 품목 과발주 방지
    var unit = 'IF(MAX(N($R' + r + '),N($U' + r + '))<10,1,' + U + ')';
    fOrder.push(['=IF(AND($R' + r + '="",$U' + r + '=""),"",MAX(0,CEILING((MAX(N($R' + r + ')-' + avail + ',N($U' + r + ')-' + avail + ')-N($V' + r + '))/' + unit + ',1)*' + unit + '))']);
  }
